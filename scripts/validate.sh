...
for path in README.md SKILL.md .codex-plugin/plugin.json .claude-plugin/plugin.json .claude-plugin/marketplace.json assets/linkedin-skills-hero.png assets/linkedin-skills-workflow.png lib/approval.py lib/config.py lib/url_parser.py scripts/selftest.py references/socialbu-publishing.md; do
...
for path in (Path('.codex-plugin/plugin.json'), Path('.codex-marketplace/linkedin-skills/.codex-plugin/plugin.json')):
...
[ "$(od -An -tx1 -N8 assets/linkedin-skills-hero.png | tr -d ' \n')" = 89504e470d0a1a0a ] || fail "bad hero PNG"
[ "$(od -An -tx1 -N8 assets/linkedin-skills-workflow.png | tr -d ' \n')" = 89504e470d0a1a0a ] || fail "bad workflow PNG"
